<div>
    ...
    <div class="breadcrumb">
        <div class="container px-3 md:px-5 xl:px-0">
            <div class="flex items-center gap-1 py-[1.5px]">
                ...
                <span class="text-[14px] font-medium leading-[110%] font-display text-gray-black inline-block"> {{
                    $productData->name }}
                </span>
            </div>
    ...
    ->exists();
    $similarProduct = \App\Models\Product::where('category_id', $productData->category_id)
        ...
        <div class="container px-3 md:px-5 xl:px-0">
            <div class="product-details-wrap pt-10">
                        ...
                        <div class="xl:flex flex-col justify-between hidden">
                            ...
                            <div class="swiper-container gallery-thumbs">
                                <div class="swiper-wrapper">
                                    @foreach ($productGallery as $galleryData)

                                    <div class="swiper-slide">
                                        <img src="{{asset('product/gallery/'.$galleryData->image)}}" alt="Slide 01">
                                    </div>
                                    @endforeach
                ...
                <div class="right-side xl:px-8 px-0 xl:w-5/12 w-full">
                    <h2 class="text-[#272343] pro-title font-semibold mb-3 capitalize"> {{ $productData->name }}
                    </h2>
                    <div class="flex items-center gap-2.5 mb-6">
                        <p class="flex gap-1.5 items-center">
                            <span class="text-[#272343] text-2xl">$ {{ $productData->price }}
                            </span>
                            <span class="text-[#272343] opacity-30 text-xl line-through">$500</span>
                        </p>
                        <span class="bg-[#F5813F] px-2.5 py-1.5 rounded-[4px] text-white text-sm"> {{
                            ...
                            <li>
                                <p class="text-[15px] inline-flex gap-2 items-center">
                                    <span class="text-[#9A9CAA]">Category:</span><span
                                        class="text-[#636270] font-medium">Wing Chair</span>
                                </p>
                            </li>
                            <li>
                                <p class="text-[15px] inline-flex gap-2 items-center">
                    ...
                    <div class="flex flex-wrap lg:flex-nowrap items-center gap-3 mb-6">
                        ...
                        <div class="flex gap-3 w-full">
                            ...
                            <div>
                                @auth
                                <a wire:click.prevent="{{ $isInWishlist ? 'RemoveFromWishlist' : 'AddToWishlist' }}({{$productData->id}},'{{$productData->name}}',{{$productData->price}})"
                        ...
                        <div
                            ...
                            <a wire:click.prevent="decreaseQuantity('{{ $rowId }}')"
                                class="w-5 h-5 inline-flex justify-center items-center text-[#9A9CAA] pl-[14px] select-none minus"
                                id="minus">-</a>
                            <input type="text" disabled class="text-[#272343] text-base plus_mines_input select-none"
                                value="{{ $qty }}" />
                            <a wire:click.prevent="increaseQuantity('{{ $rowId }}')"
    ...
    <section class="xl:py-20 py-6 md:py-10">
        <div class="container px-3 md:px-5 xl:px-0">
            <div class="flex items-center justify-between items-cente mb-10">
                <h2
                    class="text-[#272343] xl:text-[32px] xl:leading-[110%] text-xl md:text-2xl font-semibold font-display">
                    Similar Products</h2>
            ...
            <div class="swiper recentSwiper overflow-hidden">
                <div class="swiper-wrapper">
                    ...
                    @php
                    $isInWishlist2 = \App\Models\WishList::where('user_id', auth()->id())->where('product_id',
                    $product->id)->exists();
                    @endphp
                            ...
                            <a wire:click.prevent="{{ $isInWishlist2 ? 'RemoveFromWishlist' : 'AddToWishlist' }}({{$product->id}},'{{$product->name}}',{{$product->price}})"
                                ...
                                <svg width="22" height="22" viewBox="0 0 22 22" fill="none"
                                    ...
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        ...
                                        stroke="#272343" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" fill="{{ $isInWishlist2 ? 'red' : 'none' }}" />
                                </svg>
                            </a>
                            @else
